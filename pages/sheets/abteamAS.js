...
import ABASData, { allRoles } from '../../data/ABASData';
...
export default function Sheets() {
  ...
  // 提取所有可能的筛选选项
  const allSeries = Array.from(new Set(ABASData.map(work => work.series)));
  
  // 按职位分类的人员数据
  const allStaffByRole = {};
  allRoles.forEach(role => {
    allStaffByRole[role] = Array.from(
      new Set(ABASData.flatMap(work => work.staff[role] || []))
    ).sort();
  });
  
  // 所有人员数据
  const allStaff = Array.from(
    new Set(ABASData.flatMap(work => Object.values(work.staff).flat()))
  ).sort();
  ...
  // 筛选后的作品列表
  const [filteredWorks, setFilteredWorks] = useState(ABASData);
  
  // 当系列筛选变化时更新作品选项
  useEffect(() => {
    if (filters.series) {
      const seriesWorks = ABASData.filter(work => work.series === filters.series);
      ...
      setFilteredTitles(Array.from(new Set(ABASData.map(work => work.title)))); 
    }
  }, [filters.series]);
...
const scrollToTop = () => {
  ...
  return (
    <>
      <Head>
        <title>AlanBecker制作人员表格(短片)</title>
        <link rel="icon" type="image/png" href="/images/icon/C.png" />
        <meta name="description" content="AlanBecker短片作品及制作人员信息查询系统" />
      </Head>
      <ToastContainer
  ...
  >
    ↑
      ...
      <div className={styles.mainContent}>
        <i style={{color:'#ff2167'}}>对各职位进行了调整合并!影响者中Green的账号视频放在了短片里</i>
        ...
        <i style={{color:'#40e0d0'}}>欢迎指出错误和提供建议</i>
